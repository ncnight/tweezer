...
unsigned char gcm_iv[] = {0x99, 0xaa, 0x3e, 0x68, 0xed, 0x81,
                          0x73, 0xa0, 0xee, 0xd0, 0x66, 0x84};

unsigned char gcm_aad[] = {0x4d, 0x23, 0xc3, 0xce, 0xc3, 0x34, 0xb4, 0x9b,
                           0xdb, 0x37, 0x0c, 0x43, 0x7f, 0xec, 0x78, 0xde};

// Be aware that memtable key do not need to be flusehd to storage. This
// temporal key. Contents of memtable is stored in WAL.
void GenerateMemKey(void) { GenerateRandomBytes((char *)memtable_key, 32); }
...
// tags : Output
void Encryption(Slice data, unsigned char *key, unsigned char *iv,
                unsigned char *aad, unsigned char *tags) {
  EVP_CIPHER_CTX *ctx;
  ...
  EVP_EncryptUpdate(ctx, outbuf, &outlen, (const unsigned char *)data.data(),
                    data.size());
...
// data : Output
void Decryption(Slice data, unsigned char *key, unsigned char *iv,
                unsigned char *aad, unsigned char *tags) {
  unsigned char *outbuf = (unsigned char *)data.data();
  int outlen;
  ...
  EVP_DecryptUpdate(ctx, outbuf, &outlen, (const unsigned char *)data.data(),
                    data.size());
  if (tags != nullptr) {
    // tags : 16 bytes (128bits)
...
void log_encrypt(const Slice &record, char *key, char *tags, char *aad) {
  unsigned char *target_aad =
      aad ? (unsigned char *)aad : (unsigned char *)gcm_aad;
  ...
  EVP_EncryptUpdate(ctx, output, &outlen, (const unsigned char *)record.data(),
                    record.size());
  // No output
...
void log_decrypt(const Slice &record, char *key, char *tags, char *aad) {
  ...
  EVP_DecryptUpdate(ctx, outbuf, &outlen, (const unsigned char *)record.data(),
                    record.size());
